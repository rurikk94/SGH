...
public class ListaPaciente {
     ...
     * @deprecated no usar
    **/
    public void editarPaciente(Rut rut, String nombres, String apellidos, Date fechaNac, int telefono, String nombreContacto, int telefonoContacto, char sexo) {
        for (int i = 0; i < listaPacientes.size(); i++) {
            if (listaPacientes.get(i).getRut().equals(rut.toString())) {
    ...
    public String buscarPaciente(Rut rut) {
        ...
        return "No se encontro el paciente con el Rut " + rut + ".\n";
    }
    
    public boolean remplazarPaciente(Paciente pacienteNuevo, Rut pacienteARemplazar) {
        if (eliminarPaciente(pacienteARemplazar)) {
            agregarPaciente(pacienteNuevo);
            return true;
        }
        return false;
    }
